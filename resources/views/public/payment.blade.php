  ...
  <main>
    <section class="ticket">
      ...
      <div class="ticket__info-wrapper">
        ...
        <p class="ticket__info">
          Начало сеанса: <span class="ticket__details ticket__start">{{ $dateLabel ?? '' }} {{ data_get($seance, 'start_time') }}</span>
        </p>
        <p class="ticket__info">
          Стоимость: <span class="ticket__details ticket__cost">{{ $totalCost }}</span> рублей
        </p>

        <button class="acceptin-button" type="button" onclick="location.href='{{ route('client.ticket', ['seance' => data_get($seance, 'id'), 'date' => $dateKey ?? null]) }}'">
