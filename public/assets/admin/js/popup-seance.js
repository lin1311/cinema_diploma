document.addEventListener('DOMContentLoaded', function () {
    ...
    const getSeanceDuration = (movieId) => {
        const movieInfo = getMovieInfo(String(movieId));
        if (!movieInfo || !Number.isFinite(movieInfo.duration) || movieInfo.duration <= 0) {
            return null;
        }
    ...
    const findOverlapForCandidate = ({ hallId, startTime, movieId, excludeSeanceId = null }) => {
        const startMinutes = parseTimeToMinutes(startTime);
        const duration = getSeanceDuration(movieId);
        if (startMinutes === null || duration === null) {
            return { error: 'Для выбранного фильма не указана корректная длительность.' };
        }

        const endMinutes = startMinutes + duration;
        const hallSeances = Array.from(
            document.querySelectorAll(`.conf-step__seances-hall[data-hall="${hallId}"] .conf-step__seances-movie`)
        );

        for (const node of hallSeances) {
            if (excludeSeanceId && node.dataset.seanceId === excludeSeanceId) {
                continue;
            }
            const existingStart = parseTimeToMinutes(node.dataset.startTime);
            const existingDuration = getSeanceDuration(node.dataset.movieId);
            if (existingStart === null || existingDuration === null) {
                continue;
            }
            const existingEnd = existingStart + existingDuration;
            if (hasOverlap(startMinutes, endMinutes, existingStart, existingEnd)) {
                return {
                    error: `Сеанс в ${normalizeTime(startTime)} пересекается с сеансом в ${normalizeTime(node.dataset.startTime)} в этом же зале.`
                };
            }
        }
    ...
    const findAnyOverlapInGrid = () => {
        const hallBlocks = Array.from(document.querySelectorAll('.conf-step__seances-hall[data-hall]'));
        for (const hallBlock of hallBlocks) {
            const hallId = hallBlock.dataset.hall;
            const nodes = Array.from(hallBlock.querySelectorAll('.conf-step__seances-movie'));
            for (const node of nodes) {
                const result = findOverlapForCandidate({
                    hallId,
                    startTime: node.dataset.startTime,
                    movieId: node.dataset.movieId,
                    excludeSeanceId: node.dataset.seanceId
                });
                if (result.error) {
                    return result.error;
                }
    ...
    const saveSeances = () => {
        ...
        const overlapError = findAnyOverlapInGrid();
        if (overlapError) {
            updateSaveStatus(overlapError);
            return;
        }
        ...
        fetch('/admin/seances', {
            ...
            .then((data) => {
                if (!data?.success) {
                    updateSaveStatus(data?.message || 'Не удалось сохранить сеансы.');
                    return;
                }
    ...
    if (addForm) {
        addForm.addEventListener('submit', function (event) {
            ...
            const overlapCheck = findOverlapForCandidate({
                hallId,
                startTime,
                movieId: filmId
            });
            if (overlapCheck.error) {
                updateSaveStatus(overlapCheck.error);
                return;
            }
